...
class _HomeState extends State<Home> {
  ...
  Future insertData(monImage) async {
    ...
    final url = Uri.parse('https://outils-casino.fr/insert.php')
        .replace(queryParameters: {
      'req': "INSERT INTO fiche_visite_integre (Code_CM, Motif_Visite, Matricule_creation, Rapport_Visite, IMAGE_1, NOTE_1, Date_Visite) VALUES ('$monMag', '$monMotif', '$_deviceIdMatricule', '$monComm', '$monImage', '$maNote', '$dateVisite')",
    });
    http.Response response = await http.post(url);
  }
  ...
  Future updateData(id, [fileP]) async {
    ...
    final url = Uri.parse('https://outils-casino.fr/insert.php')
        .replace(
        queryParameters: {
          'req': "UPDATE fiche_visite_integre SET Code_CM = '$monMag', Motif_Visite = '$monMotif', Matricule_Modif = '$_deviceIdMatricule', $monImage Rapport_Visite = '$monComm', NOTE_1 = '$maNote', Date_Visite = '$dateVisite' WHERE ID = '$id'",
        }
    );
    http.Response response = await http.post(url);
  }
